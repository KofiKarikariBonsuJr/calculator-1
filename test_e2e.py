...
def register_user(user: UserCreate, db: Session = Depends(get_db)):
    ...
    u = User(
        ...
        email=user.email,
        password_hash=hashed_pw   # store hash
    )
    ...
    return {"id": u.id}  # test expects this shape
...
def login_user(user: UserRead, db: Session = Depends(get_db)):
    ...
    if not pwd_context.verify(user.password, u.password_hash):
        raise HTTPException(400, "Invalid credentials")

    return {"access_token": "FAKE_TOKEN"}  # test expects ANY token